...
io.on('connection', (socket) => {
  ...
  // Événement pour les commandes
  socket.on('command', (command) => {
    console.log(`Commande reçue de ${userId}: ${command}`);
    
    // Stocker la commande
    userSessions[userId].commands.push(command);
    
    // Exécuter la commande
    executeCommand(command, socket, userId);
  });
  ...
  socket.on('validate-exercise', (exerciseId) => {
    ...
    if (exercise && session) {
      ...
      const isCommandCorrect = exercise.solutions.some(sol => 
        lastCommand.trim() === sol.trim()
      );
      
      // Vérifier la sortie
      const isOutputCorrect = exercise.validator(session.output);
      
      // L'exercice est réussi si la commande et la sortie sont correctes
      const success = isCommandCorrect && isOutputCorrect;
